...
const BasketItem = ({ item }) => {
  ...
  const { title, productPrice,salePrice, images } = product || {};
  
  if (!product) {
    return null; 
  }

  const displayPrice = salePrice || productPrice || 0;
      ...
      <div className="w-full min-h-[100px] flex items-center gap-x-4">
        ...
        <div className="w-full flex flex-col">
          ...
          <div className="flex gap-x-2 h-[36px] text-sm">
            ...
            <div className="flex-1 flex items-center justify-around ">${displayPrice}</div>
            <div className="flex-1 flex justify-end items-center text-primary font-medium">{`$ ${parseFloat(displayPrice * productCount).toFixed(2)}`}</div>
          </div>
        </div>
      </div>
    </div>
